...
function Library:CreateWindow(cfg)
    ...
    Main.ClipsDescendants = true; Main.ZIndex = 10; Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 10)
    ...
    TabScroll.Size = UDim2.new(1, -45, 1, 0); TabScroll.Position = UDim2.new(0, 5, 0, 0); TabScroll.BackgroundTransparency = 1; TabScroll.ScrollBarThickness = 0; TabScroll.AutomaticCanvasSize = "X"; TabScroll.ZIndex = 21
    ...
    function Window:CreateTab(name)
        ...
        Page.Size = UDim2.new(1, 0, 1, 0); Page.BackgroundTransparency = 1; Page.Visible = false; Page.ScrollBarThickness = 0; Page.ZIndex = 16; Page.AutomaticCanvasSize = "Y"
        ...
        function Tab:CreateSection(title, side)
            ...
            local Ele = {}
            -- [ TOGGLE DẠNG NÚT GẠT (SWITCH) NHƯ CŨ ]
            function Ele:AddToggle(text, cb)
                ...
                local BG = Instance.new("Frame", TglBtn)
                BG.Size = UDim2.new(0, 35, 0, 18); BG.Position = UDim2.new(1, -45, 0.5, -9); BG.BackgroundColor3 = Color3.fromRGB(45, 45, 45); Instance.new("UICorner", BG).CornerRadius = UDim.new(1, 0)
                
                local Dot = Instance.new("Frame", BG)
                Dot.Size = UDim2.new(0, 14, 0, 14); Dot.Position = UDim2.new(0, 2, 0.5, -7); Dot.BackgroundColor3 = Color3.fromRGB(200, 200, 200); Instance.new("UICorner", Dot).CornerRadius = UDim.new(1, 0)

                local State = false
                TglBtn.MouseButton1Click:Connect(function()
                    State = not State
                    if State then
                        TweenService:Create(BG, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0, 170, 255)}):Play()
                        Dot:TweenPosition(UDim2.new(1, -16, 0.5, -7), "Out", "Quart", 0.2, true)
                        TweenService:Create(Dot, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 255, 255)}):Play()
                    else
                        TweenService:Create(BG, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(45, 45, 45)}):Play()
                        Dot:TweenPosition(UDim2.new(0, 2, 0.5, -7), "Out", "Quart", 0.2, true)
                        TweenService:Create(Dot, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(200, 200, 200)}):Play()
                    end
                    cb(State)
                end)
            end
