...
function Library:CreateWindow()
    ...
    InnerLogo.Size = UDim2.new(0, 28, 0, 28); InnerLogo.Position = UDim2.new(0, 6, 0.5, -14); InnerLogo.BackgroundTransparency = 1
    ...
    TabScroll.CanvasSize = UDim2.new(0, 0, 0, 0); TabScroll.AutomaticCanvasSize = Enum.AutomaticSize.X; TabScroll.ElasticBehavior = Enum.ElasticBehavior.Never
    ...
    local function ToggleUI()
        ...
        if not isMenuOpen then
            ...
            task.wait(0.4); Main.Visible = false; LogoBtn.Visible = true; TweenService:Create(LogoBtn, TweenInfo.new(0.4, Enum.EasingStyle.Back), {Size = UDim2.new(0, 50, 0, 50)}):Play()
        else
            TweenService:Create(LogoBtn, TweenInfo.new(0.3), {Size = UDim2.new(0, 0, 0, 0)}):Play(); task.wait(0.2); LogoBtn.Visible = false; Main.Visible = true
    ...
    function Window:CreateTab(name)
        ...
        local Left = CreateColumn(UDim2.new(0, 0, 0, 0)); local Right = CreateColumn(UDim2.new(0.5, 5, 0, 0))
        ...
        function Tab:CreateSection(title, side)
            ...
            Instance.new("UICorner", Sec).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", Sec).Color = Color3.fromRGB(35, 35, 35)
            ...
            function Ele:CreateToggle(text, def, cb)
                ...
                local Bg = Instance.new("Frame", Tgl); Bg.Size = UDim2.new(0, 36, 0, 20); Bg.Position = UDim2.new(1, -40, 0.5, -10)
                Bg.BackgroundColor3 = state and Color3.new(0, 0.8, 0.8) or Color3.new(0.2, 0.2, 0.2); Instance.new("UICorner", Bg).CornerRadius = UDim.new(1, 0)
                local Dot = Instance.new("Frame", Bg); Dot.Size = UDim2.new(0, 16, 0, 16); Dot.Position = state and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)
                Dot.BackgroundColor3 = Color3.new(1, 1, 1); Instance.new("UICorner", Dot).CornerRadius = UDim.new(1, 0)
                local Check = Instance.new("TextLabel", Dot); Check.Text = "✓"; Check.Size = UDim2.new(1, 0, 1, 0); Check.BackgroundTransparency = 1; Check.Visible = state; Check.TextColor3 = Color3.new(0, 0.5, 0.5); Check.Font = Enum.Font.GothamBold; Check.TextSize = 10
            ...
            -- [2] HÀM TẠO DROPDOWN MỚI (CHỌN VŨ KHÍ) --
            function Ele:CreateDropdown(text, options, cb)
                local index = 1
                local Drop = Instance.new("TextButton", sCont)
                Drop.Size = UDim2.new(1, 0, 0, 30); Drop.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
                Drop.Text = "   " .. text .. " : " .. tostring(options[index])
                Drop.TextColor3 = Color3.new(1, 1, 1); Drop.TextXAlignment = 0; Drop.Font = Enum.Font.GothamBold; Drop.TextSize = 12
                Instance.new("UICorner", Drop).CornerRadius = UDim.new(0, 6)
                
                -- Bấm vào để đổi Vũ Khí liên tục
                ...
                cb(options[index]) -- Gọi mặc định lần đầu
            end
